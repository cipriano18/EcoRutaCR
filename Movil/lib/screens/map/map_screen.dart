...
import 'package:ecoruta/widgets/scr_map/finish_route_sheet.dart';
import 'package:ecoruta/widgets/scr_map/guided_route_status_card.dart';
import 'package:ecoruta/widgets/scr_map/route_metrics_panel.dart';
import 'package:ecoruta/widgets/scr_map/start_route_sheet.dart';
...
class MapScreenState extends State<MapScreen> {
  ...
  Future<void> _initLocation() async {
    try {
      ...
      _currentPositionNotifier.value = currentPoint;

      _centerOnUser();
      await _refreshElevation();
      _startLocationTracking();
      _startElevationTimer();
      _updateGuidedRouteProximity(currentPoint, showFeedback: false);
    } catch (_) {
      if (!mounted) return;
      setState(() => _loading = false);
    }
    ...
    _positionSubscription =
        Geolocator.getPositionStream(
          ...
          if (_hasFreeRecording) {
            _appendFreePoint(nextPosition, position.altitude);
          }
          if (_hasGuidedRoute) {
            _updateGuidedRouteProximity(nextPosition, showFeedback: true);
            if (_isGuidedRouteStarted) {
              _appendGuidedPoint(nextPosition, position.altitude);
            }
          }
  ...
  Future<void> _attemptFinishGuidedRoute() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No se pudo guardar la sesion de esta ruta.'),
        ),
      );
    } finally {
      if (mounted) {
