...
import 'package:ecoruta/features/routes/widgets/my_route_card.dart';
  ...
  Future<void> _editRoute(StoredRoute route) async {
    if (route.isPublic) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Las rutas públicas no se pueden editar.'),
        ),
      );
      return;
    }
        ...
        child: StreamBuilder<List<StoredRoute>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              debugPrint('ERROR CARGANDO MIS RUTAS: ${snapshot.error}');
                  ...
                  ...visibleRoutes.map(
                    (route) => MyRouteCard(
                      ...
                      onDelete: _selectedTab == _MyRoutesTab.saved
                          ...
                          : () => _removeRoute(route),
                      deleteActionLabel: _selectedTab == _MyRoutesTab.saved
                          ? 'Quitar'
                          : 'Eliminar',
                      onEdit:
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Todavía no has guardado rutas públicas de otros usuarios desde Explorar.',
            textAlign: TextAlign.center,
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Ajusta los filtros o guarda una ruta desde Explorar para verla aquí.',
            textAlign: TextAlign.center,
            style: TextStyle(color: textMuted, height: 1.45),
          ),
        ],
