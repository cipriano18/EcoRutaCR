...
import 'package:ecoruta/features/routes/widgets/save_route_sheet.dart';
...
class _GenerateTabState extends State<GenerateTab> {
  ...
  bool _isUsingCurrentLocationAsStart = true;
  ...
  /// Sincroniza el origen sugerido con la ubicación compartida de la app.
  void _handleSharedLocationChanged() {
    if (!mounted) return;

    final currentPoint = _locationProvider?.currentLatLng;
    final isLoading = _locationProvider?.isLoading ?? false;

    if (currentPoint != null) {
      _currentLocation = currentPoint;
      _isLoadingCurrentLocation = isLoading;
      if (_isUsingCurrentLocationAsStart) {
        setState(() {
    ...
    setState(() {
      if (!isLoading) {
        _startPoint = null;
        _isUsingCurrentLocationAsStart = true;
  ...
  void _swapLocations() {
    setState(() {
      ...
      _isUsingCurrentLocationAsStart = false;
    });
    _syncPreviewMap();
  }
  ...
  Future<void> _openPointsPicker() async {
    ...
    final sharedLocation = _locationProvider?.currentLatLng;
    setState(() {
      ...
      _isUsingCurrentLocationAsStart = _isSamePoint(
        result.startPoint,
        sharedLocation,
      );
    });
  ...
  Future<void> _generateRoutes() async {
    ...
    final challengingRoute = provider.routes[RoutingPreference.mostChallenging];
