...
import 'package:ecoruta/features/routes/screens/picker_map.dart';
...
import 'package:ecoruta/features/routes/widgets/activity_type_card.dart';
import 'package:ecoruta/core/widgets/confirm_dialog.dart';
import 'package:ecoruta/features/routes/widgets/points_preview.dart';
...
class _SearchTabState extends State<SearchTab> {
  ...
  void _handleSharedLocationChanged() {
    ...
    final nextLocation = _locationProvider?.currentLatLng;
    final previous = _lastSharedLocation;
    _lastSharedLocation = nextLocation;

    if (previous == null && nextLocation == null) return;
    if (previous != null &&
        nextLocation != null &&
        previous.latitude == nextLocation.latitude &&
        previous.longitude == nextLocation.longitude) {
      return;
    }
    ...
    final result = await Navigator.of(context).push<PointsSelectionResult>(
      MaterialPageRoute(
        builder: (_) => PickerMapScreen(
          ...
          currentLocation: _locationProvider?.currentLatLng,
  ...
  Future<void> _searchPublicRoutes() async {
    ...
    try {
      ...
      final savedSourceRouteIds = await _savedRoutesService
          .fetchSavedPublicSourceRouteIds();
      ...
      setState(() {
        _searchErrorMessage =
            'No se pudieron consultar las rutas públicas en este momento.';
        _publicRoutes = const [];
      });
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    final destinationPreviewLabel =
        context.watch<LocationProvider>().isLoading && _destinationPoint == null
    ...
    return ListView(
      ...
      children: [
        ...
        PointsPreview(
          ...
          previewCenter: context.watch<LocationProvider>().currentLatLng,
          startLabel: '',
        ...
        if (_searchErrorMessage != null)
          ...
          const _InfoCard(
            message:
                'Selecciona un destino, ajusta el radio y busca rutas públicas cercanas.',
            icon: Icons.travel_explore_rounded,
            iconColor: _primaryColor,
          )
        else if (_isSearchingRoutes)
          const _InfoCard(
            message: 'Buscando rutas públicas dentro del radio seleccionado...',
          ...
          const _InfoCard(
            message:
                'No se encontraron rutas públicas dentro del radio elegido para esta actividad.',
          ...
          ..._publicRoutes.map(
            (routeData) => Padding(
              ...
              child: RouteResultCard(
                ...
                isSecondaryLoading: _isSavingPublicRoute,
                onSecondaryPressed: _isSavingPublicRoute
                    ? null
  ...
  String _resultsSummaryText() {
    if (!_hasSearchedRoutes) {
      return 'Todaví­a no has consultado rutas públicas para este destino';
    }
    if (_isSearchingRoutes) {
      return 'Consultando rutas de ${_selectedProfileLabel.toLowerCase()} cerca del destino seleccionado';
    }
    if (_searchErrorMessage != null) {
      return 'La consulta de rutas públicas no pudo completarse';
    }
    return 'Se encontraron ${_publicRoutes.length} rutas públicas de ${_selectedProfileLabel.toLowerCase()} dentro de ${_radius.toInt()} km';
  }
  ...
  Future<void> _confirmSavePublicRoute(_PublicRouteCardData routeData) async {
    ...
    setState(() => _isSavingPublicRoute = true);
    try {
      ...
      if (mounted) {
        setState(() => _isSavingPublicRoute = false);
      }
    }
  }
  ...
  void _syncPreviewMap() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final center = _destinationPoint ?? _locationProvider?.currentLatLng;
      if (center != null) {
