...
class OsmGraphData {
  ...
  /// Nodos válidos extraídos de la respuesta de Overpass.
  final List<GeoNode> nodes;

  /// Aristas dirigidas construidas desde las ways.
  final List<GeoEdge> edges;

  /// Ways crudas conservadas para depuración o inspección.
...
class OsmMapper {
  ...
  OsmGraphData mapToGraph(
    ...
    for (final element in elements) {
      ...
      for (var i = 0; i < validNodeIds.length - 1; i++) {
        ...
        // Cada segmento se convierte en una o dos aristas dirigidas según la
        // restricción `oneway` de OSM.
        if (!isReverseOneway) {
          edges.add(
            GeoEdge(
  ...
  /// Convierte tags dinámicos de OSM a un mapa de texto seguro.
  ...
  /// Calcula distancia aproximada entre dos nodos usando Haversine.
